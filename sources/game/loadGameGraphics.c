...
{
	graphics->arialFont = loadFont(gameLogs, FREE_FONT_PATH, 20);
...
{
    TTF_Font *font = TTF_OpenFont(FREE_FONT_PATH,20);
	if( font )
	{
		...
		loadSingleTextTexture(gameLog, renderer, &tabTextes[PUSH_SPACE], "Appuyez sur 'espace' ou le bouton droit de la souris pour cacher les cartes", font, whiteColor);
		
		TTF_CloseFont(font);
	}
	else{
		fprintf(gameLog, "Error: couldn't load '%s' font due to error: %s", FREE_FONT_PATH, TTF_GetError() );
	}
